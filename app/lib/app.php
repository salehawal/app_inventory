...
function add_user()
{
	...
	if(strlen(trim($_POST['password'])) > 0)
	{
		...
		$q 		= "insert into fict_users (fu_code, fu_username, fu_password, fu_cre_date) values (:fu_code, :fu_username, :fu_password, :fu_cre_date)";
		$st 	= $sysdata['conn']->prepare($q);
		$user 	= clean_sql($_POST['username']);
		$pass 	= clean_sql($_POST['password']);
		print_r(array($uid,$user,$pass,date('Y-m-d'))); exit;
		$st->bindValue(':fu_code',$uid);
		$st->bindValue(':fu_username',$user);
		$st->bindValue(':fu_password', $pass);
		$st->bindValue(':fu_cre_date', date('Y-m-d'));
		$st->execute();
		return $uid;
	}
	else
